...
The basic workflow of \Rpackage{powsim} is illustrated in figure \ref{fig:powsim_scheme}.
...
TwoiLIF.params = estimateNBParam(countData = Kolodziejczyk.cnts,
                               cData = NULL,
                               design = NULL,
                               RNAseq = 'singlecell',
...
\begin{figure}
\centering
\includegraphics[width=0.98\linewidth]{NBparams.jpeg}
...
\begin{figure}
\centering
\includegraphics[width=0.8\linewidth]{simeval.jpeg}
...
% \begin{figure}
% \centering
% \includegraphics[width=0.75\linewidth]{lfcdist.jpeg}
% \caption{\textbf{Log2 fold change examples for gamma, uniform and normal distribution}
% \label{fig:lfcs}
% \end{figure}
...
sim.opts = SimSetup(desetup = de.opts, params = 2iLIF.params, size.factors='given')
@

With the setup defined, the differential expression simulation is run with \function(simulateDE). For this, the user needs to set the following options:

\begin{description}
  \item[Replicates] The number of sample replicates per group (n1 and n2). These can be unbalanced.
  \item[DEmethod] The differential testing method. The user can choose between  methods in total.  developed for bulk, xx developed for single cells. A detailed list can be found under point xx.
...
%\begin{figure}[h]
%\centering
%\includegraphics[width=0.75\linewidth]{evalres.jpg}
%\caption{\textbf{Power assessment for Kolodziejczyk simulation.}
%\label{fig:power}
%\end{figure}
...
<<echo=TRUE,eval=FALSE>>=
2iLIF.distfit = evaluateDist(cnts = Kolodziejczyk.cnts,
                             RNAseq = 'singlecell',
                             ncores = 1,
                             nsims = 1,
                             frac.genes = 1,
...
plotEvalDist(EvalDist, annot=F)
@

%\begin{figure}[h]
%\centering
%\includegraphics[width=0.75\linewidth]{lfc_dists.jpg}
%\caption{\textbf{Evaluation of distribution fitting for Kolodziejczyk RNA-seq experiment.}
%\label{fig:evaldist}
%\end{figure}
...
We have also implemented the option to approximate the read count matrix simulation based on random distribution functions in \R{}. The user then has to define the mean, dispersion, dropout and library size in \Rfunction{insilicoNBParam}. In the absence of suitable pilot studies,
For example, a typical single cell RNA-seq experiment could be approximated with:
\begin{itemize}
  \item mean
  \item dispersion
  \item library size
...
\subsubsection{Precalculated Parameter Estimates}

We have included precalculated parameter estimates for bulk as well as single cell RNA-seq experiments.
The data sets included represent different levels of mean, dispersion and dropout characteristics.
The user can view these estimates with \Rfunction{plotNBParam} and use it as an input for \Rfunction{SimSetup}.
